...
* `wrapped_objects` is a list of objects, each having at least one mandatory and up to three properties:
  * `original_name` (_mandatory_) - the original name of the object to be wrapped. Do not mention `window` here!
	* `wrapped_name` - the variable name that can be used by `wrapping_function_body`, `helping_code`
	...
	* `callable_name` - like `wrapped_name`, this creates a variable referring to the original object and available only
		in the code generate by this wrapper, but it's specifically **meant to be used for native methods and functions**
		which your code needs to call. This is especially important if they **accept callback arguments or return `Promise` objects**:
